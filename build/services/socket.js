...
var webSocket = function webSocket() {
  var usuarios = [];
  var myWSServer = (0, _socket["default"])(_server["default"]);
  myWSServer.on('connection', function (socket) {
    _logger.logger.info('un cliente se a conectado');

    _logger.logger.info('socket del server', socket.id);

    _logger.logger.info('socket del cliente', socket.client.id);

    var usuario = socket.client.id;
    usuarios.push(usuario);
    socket.emit('newYser', usuario);
    socket.on('message', function (data) {
      (0, _productos.save)(data);
      var productos;
